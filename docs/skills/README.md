...
| Skill | Status |
|---|---|
| `autotask-time-entry-writer` | EDITED — `companyLocationID` added to the field list, the three fields a company change moves with it documented (site location resolved, stale contact and contract cleared), and a note that an Autotask HTTP 500 with a structured `errors[]` body is a request rejection and must not be retried |
| `autotask-troubleshooting-advisor` | EDITED — the hand-off for a mis-filed ticket now states what else a company change moves, and asks for the right site and contact up front |
| `service-delivery-manager` | EDITED — ticket-hygiene defect list gains wrong site location, and the wrong-company finding now says the fix also clears a stale contact and contract. Still read-only; it names defects and hands the write to `autotask-time-entry-writer` |
