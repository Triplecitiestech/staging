...
Three things to state when you hand over a wrong company, so nobody is surprised by them. Autotask refuses a company change while the old customer's location or contact is still attached, so the fix moves more than the company. (1) The ticket's SITE LOCATION becomes the new company's primary, or is cleared if it declares none. (2) Its CONTACT is CLEARED if it carries one from the old company - and a mis-filed ticket usually does have a contact, so expect this. (3) A CONTRACT belonging to the old customer BLOCKS the move outright and cannot be cleared through the API, so the fix is refused until someone either names a contract at the new company or clears the field in the Autotask UI. If you know which site the work belongs to, or the right contact and contract at the new company, say so in the hand-off so the writer skill can set them in the same call.
