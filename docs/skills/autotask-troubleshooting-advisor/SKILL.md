...
One thing to state when you hand over a wrong company, so nobody is surprised by it: a company change also moves the ticket's SITE LOCATION. Autotask stamps a `companyLocationID` on every ticket and refuses the whole change while it still points at the old customer, so the writer skill resolves the new company's primary location - or clears the field when that company declares none. If the customer has several sites and you know which one the work belongs to, say which, so the writer can set it explicitly instead of taking the primary.
