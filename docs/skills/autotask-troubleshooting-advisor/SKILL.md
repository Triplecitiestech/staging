...
Three things to state when you hand over a wrong company, so nobody is surprised by them. Autotask refuses a company change while the old customer's location or contact is still attached, so the fix moves more than the company. (1) The ticket's SITE LOCATION becomes the new company's primary, or is cleared if it declares none. (2) Its CONTACT is CLEARED if it carries one from the old company - and a mis-filed ticket usually does have a contact, so expect this. (3) Its CONTRACT is NOT fixed: a stale contract does not block the change and cannot be cleared through the API, so the ticket can end up billed against the old customer's contract and a human has to correct that in Autotask. If you know which site the work belongs to, or the right contact and contract at the new company, say so in the hand-off so the writer skill can set them in the same call.
