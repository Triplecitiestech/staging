...
- **`[skip-e2e]` in the commit SUBJECT (first line) skips only the e2e gate** — for pipeline emergencies (Vercel GitHub integration down, no preview deployments being created). Body text is deliberately ignored: the first version checked the whole message and a commit *describing* the flag accidentally skipped its own e2e run. Secret-scan and quality gates can never be skipped. Record any use in `docs/session-summary.md`.
...
- **The e2e gate runs with `--workers=4`** (CLI override of the config's CI `workers: 1`). Serial execution took >42 min against the preview and hit the 45-min job timeout (run 1057, correctly blocking the merge). Don't remove the override; if the suite outgrows the 60-min job timeout, raise workers before raising the timeout.
...
- **Vercel Deployment Protection is ENABLED on previews** (discovered run 1059: every request from CI hit the Vercel SSO wall — 401s on pages, HTML login page on API calls, blocked `/api/test/auth`). e2e against previews requires the **Protection Bypass for Automation** secret: Vercel project → Settings → Deployment Protection → Protection Bypass for Automation; the same value must exist as the `VERCEL_AUTOMATION_BYPASS_SECRET` GitHub Actions secret. `playwright.config.ts` sends it via `x-vercel-protection-bypass` + `x-vercel-set-bypass-cookie` headers (no-op when the env var is absent). Do NOT disable Deployment Protection instead — protected previews are the better security posture.
...
- **Empty phases after sync**: If phases show up empty, the task API likely failed silently. Use `?step=resync` to re-fetch and check `?step=diagnose` for API errors. Resync also cleans up genuinely empty phases and updates phase statuses based on task completion.
...
1. **Auto-deploy from all branches** — Vercel auto-deploys preview environments from every push. (Previews are no longer publicly accessible: Vercel Deployment Protection now fronts them — confirmed 2026-06-09. Automation reaches them via the protection-bypass secret.)
