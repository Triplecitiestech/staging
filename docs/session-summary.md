...
- **Known gaps / follow-ups**: Thread → form-link webhook (`/api/integrations/thread/webhook`) exists but isn't wired up in Thread; no admin UI for generating pre-filled form links (`POST /api/forms/links` is session/secret-authed API only); true automation of mailbox conversion/forwarding would need an Exchange Online integration (certificate + `Exchange.ManageAsApp` + a PowerShell-capable runner — not possible from the Vercel JS runtime); consider whether license removal should be deferred when `keep_accessible` is chosen so conversion isn't racing the mailbox grace period.
