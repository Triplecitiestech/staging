...
> **Last updated**: 2026-07-05. Offboarding automation: requested-action reconciliation (no more silently omitted actions, e.g. shared-mailbox conversion) + IT Glue SOPs rewritten to portal-first.
...
## Exchange Online automation: shared-mailbox conversion + delegation (2026-07-05, same session) — branch `claude/offboarding-automation-issues-hfrje4` (restarted from main after PR #131 merged)

- **Goal**: actually execute "convert to shared mailbox + grant access" instead of leaving it `[MANUAL]`. Graph can't do it; the supported path (verified against Microsoft Learn 2026-03/2026-02 pages) is EXO PowerShell v3 app-only cert auth (`Office 365 Exchange Online -> Exchange.ManageAsApp`, multitenant app, per-tenant admin consent + per-tenant role for the SP — default **Exchange Recipient Administrator**, NOT Exchange Administrator). Vercel can't run PowerShell → **Azure Automation runbook** bridge.
- **Key Microsoft rule now enforced in the pipeline**: a mailbox must still be LICENSED to convert it to shared — conversion runs BEFORE license removal, and license removal is **deferred** (explicit `[MANUAL]` follow-up) whenever a conversion is requested but not confirmed done (including the not-enabled manual path — previously we removed licenses first and told techs to re-license if blocked, which was backwards).
- **Pieces**: `src/lib/exchange-online.ts` (dispatcher: kill switch `EXO_AUTOMATION_ENABLED` + per-tenant allowlist `EXO_ENABLED_TENANTS`, Graph lookup of the tenant's `.onmicrosoft.com` domain, per-job HMAC callback tokens — no shared callback secret); raw-pg `exo_jobs` table (in `/api/migrations/run` — POST after deploy); `POST /api/webhooks/exo-jobs` (HMAC-verified callback: updates job, records hr_request_steps, posts confirmation/failure notes when the pipeline stopped waiting); pipeline waits ≤120 s → `[DONE]`/`[FAILED]`/`[QUEUED]` via the reconciliation (never done-at-dispatch); runbook `scripts/exo/Invoke-TctExoOffboardingJob.ps1` (PS 7.2, idempotent, verifies OBSERVED state, reports size/litigation-hold warnings); per-tenant enablement script `scripts/exo/Enable-TenantExoAutomation.ps1`; full setup + pilot checklist in `docs/reference/EXO_AUTOMATION.md`.
- **Ships disabled**: without env vars, behavior is identical to the merged PR #131 state except license removal is now (correctly) deferred on keep_accessible offboardings. 251 unit tests green.
- **Operator setup required**: app registration + cert + Automation account + webhook + env vars + per-tenant consent/role + `/api/migrations/run` POST — numbered in `docs/reference/EXO_AUTOMATION.md`.
