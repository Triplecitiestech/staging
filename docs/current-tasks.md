...
- [ ] **[2 — RUN THE DRIFT REPORT]** Ask Claude to run `autotask_surface_drift_report`. It diffs the connector's write allowlists against live Autotask metadata and returns a tracked backlog — fields we allow that the API rejects, and writable fields we don't yet expose. This is the payoff tool: it tells you what the connector is silently wrong about. Worth reading once now and re-running after any Autotask upgrade.
