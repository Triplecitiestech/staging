...
- [x] **FR3** HIPAA Security Rule evaluators (`src/lib/compliance/frameworks/hipaa.ts`) — 32 implementation specifications across Administrative (§164.308), Physical (§164.310), and Technical (§164.312) Safeguards. Evaluators delegate to CIS v8 where the underlying check is identical (encryption, audit, backup, MFA, anti-malware, account management); pure policy/organizational controls (Risk Analysis, Sanction Policy, BAAs, workforce clearance, facility security) are `evaluationType: 'manual'` and surface as `needs_review` for engineer attestation. Engine wired up — assessments against `frameworkId: 'hipaa'` now produce real findings instead of empty rows. ✅ 2026-05-13
