...
> **Last updated**: 2026-07-29d. Latest: **Kaseya Quote Manager wired into the MCP connector (read-only, 20 tools / all 39 operations)** — no migration; **reconnect the connector** and run `kqm_probe_connection` once, steps below. Previous (2026-07-29c): **three live-reproduced connector defects fixed** on branch `claude/tct-mcp-connector-fixes-lb7e5k` — no migration, no reconnect; two live checks listed below. Previous (2026-07-29b): **drift report run + the parent-id false-report fix it exposed** on branch `claude/tct-mcp-failure-reasons-ew5tz0` — no migration, no reconnect. Previous (2026-07-29): **MCP connector failure-reason envelope + Autotask capability layer** (merged PRs #170, #171, #174) — merged, deployed and live-verified. Reconnect the connector once so the two new tools appear, steps below. Previous (2026-07-28): **Read-only sales-pricing connector tools** on branch `claude/sales-pricing-mcp-tool-3qtl1e` — no migration needed; reconnect the connector once so the two new tools appear, steps below. Previous (2026-07-28): **Connector usage telemetry + `/admin/connector/usage`** on branch `claude/connector-telemetry-dashboard-glcz62` — one migration POST then live verification, steps below. Previous (2026-07-20): **Human-vs-automated ticket classification** shipped on branch `claude/tct-automated-alerts-reporting-e4s0qr` — support metrics now count human tickets only; automated alerts report as "Security & Monitoring Activity". Post-deploy verification below. Previous (2026-07-18): **Datto RMM read-only reporting tools** (17 `datto_rmm_*` tools) added to the MCP connector on branch `claude/datto-rmm-reporting-tools-ngxkzy` — operator verification steps below. Previous (2026-07-16): **IT Glue document-folder tools** unified on `claude/itglue-document-folders-lf5xa9`; pending operator step below. Earlier pending items may still be open.

## Kaseya Quote Manager connector tools: what's left (2026-07-29)

Step 2 of 2 — the tools are built. **No migration** (no schema changes). `KASEYA_QUOTE_MANAGER_API_KEY` already exists in Vercel; this deploy is what makes it visible to the running app, since a variable added after a deployment is not exposed to it.

**Nothing about live API behaviour is confirmed yet.** The whole surface is built from the captured spec. Step 2 below is what turns that into a working integration — until it passes, treat the integration as untested.

- [ ] **[1 — REFRESH THE CONNECTOR TOOL LIST]** 21 NEW tool names shipped, so a client holding a cached tool list will not see them. In Claude, open connector settings and disable → re-enable the Triple Cities Tech connector. URL unchanged (`https://www.triplecitiestech.com/api/connector/entra/mcp`), so no re-auth needed.
- [ ] **[2 — SETTLE THE AUTH CONTRADICTION]** Ask Claude to run **`kqm_probe_connection`**. Kaseya's own docs disagree with themselves — the OpenAPI spec says the API key goes in a **header**, the help page says a **query parameter** — so this is genuinely unknown, not something I chose to leave open. The probe tries each mechanism *in isolation* (never both, which would make a success uninterpretable) and reports which authenticates.
  - `working: "header"` → the spec was right, it is already the default, nothing to do.
  - `working: "query"` → set **`KASEYA_QUOTE_MANAGER_AUTH_MODE=query`** in Vercel and redeploy. Tell me and I will make it the code default.
  - `working: null` → the key itself is the likely problem (wrong value, not yet active, or the deploy predates the variable). Do not assume the mechanism is wrong until a known-good key has been tried.
- [ ] **[3 — READ SOMETHING REAL]** Ask for your Quote Manager quotes, or a specific quote by number. Expect `mode: "list"`, a `count`, and either `truncated: false` or an explicit truncation note. Sanity check one quote against the Quote Manager UI — this is the first time any of this has touched the live API.
- [ ] **[4 — CONFIRM THE UNDOCUMENTED CAP]** In the probe output, `pageSizeCapHonoured` should be `true`. The 100-row cap comes from the help page only; the spec never states a maximum.
- **Known limits, by the vendor not by us:** products are searchable by `manufacturerPartNumber` **exact match only** (no name search exists), quotes only by `quoteNumber` (no customer or date-range filter), and quote lines filter by `quoteSectionID` — not quote id, so sections resolve first. `modifiedAfter` works on only 15 of 20 resources, so no delta sync for categories, employees, product images, quote lines or quote sections. `productimage` has no get-by-id. These are API facts, not gaps I can close.
- **No write tools exist and none can be added** — all 39 documented operations are GET, so the API has no write surface. If you need to *create* a quote, that has to happen in the Quote Manager UI. Previous (2026-07-29): **MCP connector failure-reason envelope + Autotask capability layer** (merged PRs #170, #171, #174) — merged, deployed and live-verified. Reconnect the connector once so the two new tools appear, steps below. Previous (2026-07-28): **Read-only sales-pricing connector tools** on branch `claude/sales-pricing-mcp-tool-3qtl1e` — no migration needed; reconnect the connector once so the two new tools appear, steps below. Previous (2026-07-28): **Connector usage telemetry + `/admin/connector/usage`** on branch `claude/connector-telemetry-dashboard-glcz62` — one migration POST then live verification, steps below. Previous (2026-07-20): **Human-vs-automated ticket classification** shipped on branch `claude/tct-automated-alerts-reporting-e4s0qr` — support metrics now count human tickets only; automated alerts report as "Security & Monitoring Activity". Post-deploy verification below. Previous (2026-07-18): **Datto RMM read-only reporting tools** (17 `datto_rmm_*` tools) added to the MCP connector on branch `claude/datto-rmm-reporting-tools-ngxkzy` — operator verification steps below. Previous (2026-07-16): **IT Glue document-folder tools** unified on `claude/itglue-document-folders-lf5xa9`; pending operator step below. Earlier pending items may still be open.
