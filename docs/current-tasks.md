...
- [x] **W7** Per-customer cockpit landing page (`src/app/admin/compliance/[companyId]/page.tsx`). Server-rendered, read-only summary: bootstrap progress (4 stages), latest assessment per framework with a posture score, finding counts (pass/fail/review) from the most-recent complete assessment per framework, change queue (drafted/bundled/awaiting_customer/scheduled/in-flight + active bundle count), needs-attention list from stale-disposition surfacing, and recommended frameworks from the Customer Profile auto-detect. Mobile-responsive (grid-cols-1 / sm:grid-cols-2 / lg:grid-cols-4). All data fetched in parallel via direct DB queries (no HTTP round-trip). Forbidden-color audit clean. `tests/e2e/compliance-cockpit.spec.ts` verifies unauth visitors are redirected. The single-fetch endpoint mentioned in the original plan is unnecessary because the server component fetches directly. ✅ 2026-05-13
