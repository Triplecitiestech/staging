...
*Last updated: 2026-07-16 · Owner: Kurtis (kurtis@triplecitiestech.com)*
...
- `2026-07-16` — **IT Glue connector gained document-folder tools** so connector-created SOPs stop defaulting to the org root: `itglue_list_document_folders`, `itglue_create_document_folder`, `itglue_move_document` (in `src/lib/mcp-itglue-tools.ts` + `src/lib/it-glue.ts`; move reuses the existing `updateDocument` PATCH of `document_folder_id`, no parallel client), and `itglue_create_document` now instructs callers to resolve the folder first. Folder list+create are public IT Glue API (verified against the developer docs); folder attrs are dasherized (`parent-id` null = top-level). Field notes: `docs/gotchas.md` → IT Glue.
