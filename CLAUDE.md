...
- `2026-07-16` — **MCP connector gained IT Glue document-folder tools**: `itglue_list_document_folders` (resolve folder ids — never guess), `itglue_create_document_folder` (public API DOES support folder create; delete stays UI-only), `itglue_move_document` (PATCH `document_folder_id`, read-back verified). `itglue_create_document` now instructs folder-first placement — Claude-created SOPs must stop landing at the org root. Folder endpoints are org-nested only; the folders index default (`omit` = ALL) is the opposite of the documents index (`omit` = root-only). Field notes: `docs/gotchas.md` → IT Glue.
