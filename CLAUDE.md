...
- **SaaS Alerts API is Cloudflare-blocked**: The SaaS Alerts REST API (`manage.saasalerts.com/api`) is behind Cloudflare bot protection. ALL server-to-server requests return 403 Cloudflare challenge pages regardless of auth headers. Do NOT attempt to fix this by trying different auth patterns — it's a Cloudflare block, not an auth issue. The only integration path is webhooks. Domain `www.triplecitiestech.com` is approved. Webhook receiver is at `/api/compliance/webhooks/saas-alerts`. Pending Kaseya support to configure webhook delivery.
