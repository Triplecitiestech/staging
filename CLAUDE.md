...
*Last updated: 2026-07-05 · Owner: Kurtis (kurtis@triplecitiestech.com)*
...
- `2026-07-05` — **Shared-mailbox conversion + delegation is now automatable** via an Azure Automation runbook (EXO PowerShell app-only cert auth; Graph has no API for it): `src/lib/exchange-online.ts` + `scripts/exo/` + `POST /api/webhooks/exo-jobs`, ships DISABLED (`EXO_AUTOMATION_ENABLED` + `EXO_ENABLED_TENANTS` allowlist). Pipeline converts BEFORE license removal and defers license removal until conversion is confirmed (a mailbox must be licensed to convert — Microsoft rule); jobs are marked done only by the runbook's observed-state callback. Setup: `docs/reference/EXO_AUTOMATION.md`. Forwarding = phase 2, still `[MANUAL]`.
