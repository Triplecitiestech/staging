...
interface NewProjectFormProps {
  ...
  userEmail: string
  aiPhases?: AIPhase[]
  onProjectInfoChange?: (info: { title?: string; companyId?: string }) => void
}

export default function NewProjectForm({ companies, templates, userEmail, aiPhases, onProjectInfoChange }: NewProjectFormProps) {
  ...
  // Notify parent of project info changes for AI context
  useEffect(() => {
  ...
  }, [formData.title, formData.companyId, onProjectInfoChange])
    ...
    try {
      const response = await fetch('/api/projects', {
        ...
        body: JSON.stringify({
          ...
          aiPhases: aiPhases && aiPhases.length > 0 ? aiPhases : undefined,
        }),
      })
      ...
      {aiPhases && aiPhases.length > 0 && (
        <div className="bg-gradient-to-br from-purple-600/20 to-purple-500/10 backdrop-blur-sm border border-purple-500/30 rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-purple-300">AI-Generated Project Structure</h3>
          </div>
          <p className="text-sm text-purple-200 mb-3">
            {aiPhases.length} phase{aiPhases.length !== 1 ? 's' : ''} will be created with this project
          </p>
