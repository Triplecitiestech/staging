...
export function Catalog({ showInternal, quote }: { showInternal: boolean; quote: PackageQuote }) {
  ...
  const includedCount = services.filter((s) => serviceInclusionState(s, quote.packageId) === "included").length;
  const billableCount = services.filter((s) => serviceInclusionState(s, quote.packageId) === "billable").length;

  return (
    <Card
      title="Service Catalog"
      subtitle={`${quote.packageName} — ${includedCount} of ${services.length} capabilities included${billableCount ? `, ${billableCount} available hourly (T&M)` : ""}. ${showInternal ? "Internal view shows products & vendors." : "Customer-facing capabilities."}`}
    >
      {pkg?.supportModel && (
        ...
        {unitOrder.filter((u) => grouped[u]).map((unit) => (
          <div key={unit}>
            ...
            <ul className="space-y-1.5">
              {grouped[unit].map((s) => {
                const state = serviceInclusionState(s, quote.packageId);
                return (
