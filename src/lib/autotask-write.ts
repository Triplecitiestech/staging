...
async function post<T = unknown>(path: string, body: unknown, imp?: number): Promise<T> {
  const res = await fetch(`${baseUrl()}/${path}`, { method: 'POST', headers: writeHeaders(imp), body: JSON.stringify(body) })
...
async function patch<T = unknown>(path: string, body: unknown, imp?: number): Promise<T> {
  const res = await fetch(`${baseUrl()}/${path}`, { method: 'PATCH', headers: writeHeaders(imp), body: JSON.stringify(body) })
  const text = await res.text()
