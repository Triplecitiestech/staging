...
export function registerDattoRmmTools(server: any) {
  ...
  // ── Activity / audit logs ────────────────────────────────────────────────
  server.registerTool('datto_rmm_activity_logs', { title: 'Datto RMM: activity/audit logs', description: 'Read-only, and in fact reads nothing: WITHDRAWN — THIS TOOL RETURNS NO DATA, and is kept registered only so the reason is discoverable instead of guessable. It reads NOTHING and always refuses. TWO THINGS TO KNOW. (1) IT NEVER HELD DEVICE ONLINE/OFFLINE HISTORY. If you want to know when a device went down or came back, or how long a site was dark, this endpoint is not the source and never was — use datto_rmm_alerts (an offline alert with its open and resolved timestamps), datto_rmm_get_device (lastSeen / online status now), or the Datto RMM console. (2) ITS OUTPUT WAS NOT TRUSTWORTHY: asked for a fifteen-day window on 2026-09-09 it returned a single row carrying a 1970 timestamp, i.e. an epoch-zero date, which is a value no real activity can have. A tool that returns something answer-shaped that is not an answer is worse than no tool, so it refuses rather than being repaired — repairing it was deliberately ruled out, since the audit log it reads is not something TCT needs from the connector. For who-did-what auditing, use the Datto RMM console directly.', inputSchema: { from: z.string().optional().describe('UTC window start (YYYY-MM-DD or ISO datetime)'), until: z.string().optional().describe('UTC window end (YYYY-MM-DD or ISO datetime)'), entities: z.array(z.enum(['device', 'user'])).optional().describe('Entity types to include'), categories: z.array(z.string()).optional().describe('Category filters (e.g. audit, monitor, job)'), actions: z.array(z.string()).optional().describe('Action filters'), siteIds: z.array(z.number().int()).optional().describe('Numeric site ids (NOT UIDs)'), userIds: z.array(z.number().int()).optional().describe('Numeric user ids'), searchQuery: z.string().optional().describe('Free-text query (supports wildcards/boolean per Datto docs)'), size: z.number().int().min(1).max(250).optional().describe('Rows to return (default 50)'), order: z.enum(['asc', 'desc']).optional().describe('By creation date (default desc)'), searchAfter: z.array(z.string()).optional().describe('Cursor from the previous response\'s nextSearchAfter'), page: z.enum(['next', 'previous']).optional().describe('Cursor direction, used with searchAfter') } },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async ({ from, until }: any) => {
      // Deliberately NO request. Issuing one would only re-manufacture the
      // ambiguity: a single row with an epoch-zero timestamp looks like data.
      return failureResult({
        reasonCode: 'UPSTREAM_UNSUPPORTED',
        message:
          'datto_rmm_activity_logs is withdrawn and read nothing. It does NOT hold device online/offline history — it never did — and in the one live test its own output was not trustworthy: a fifteen-day window returned a single row with a 1970 (epoch-zero) timestamp.',
        evidence:
          'Reproduced live on 2026-09-09: a ~15-day from/until window returned exactly one activity row whose date was the Unix epoch, a value no real activity can carry. Separately, the Datto RMM activity-log endpoint carries platform audit entries (who did what in the console), not device up/down transitions, so it was never the right source for connectivity history.',
        remediation:
          'For device up/down history use datto_rmm_alerts (an offline alert carries its raised and resolved timestamps) or datto_rmm_get_device for current status and lastSeen. For a whole site being dark, the Domotz collector-connectivity report is the truer signal. For console auditing — who changed what in Datto RMM — use the Datto RMM console directly; that need does not justify a connector tool that returns answer-shaped non-answers.',
        surface: 'datto_rmm',
        tool: 'datto_rmm_activity_logs',
        details: { requestedFrom: from ?? null, requestedUntil: until ?? null, requestIssued: false },
      })
    })

  // ── Jobs (results of already-run jobs; nothing here can run one) ─────────
  server.registerTool('datto_rmm_job_status', { title: 'Datto RMM: job status/results', description: 'Read-only. Status and results of an ALREADY-RUN job by job UID (this tool cannot create or run jobs). Returns the job record (name, status, created) and its components. Pass deviceUid to add that device\'s per-component results, and includeOutput=true to also fetch captured StdOut/StdErr for that device. Find job UIDs in the Datto RMM console (the activity log tool that used to be suggested here is withdrawn — see datto_rmm_activity_logs for why).', inputSchema: { jobUid: z.string().describe('Job UID'), deviceUid: z.string().optional().describe('Device UID to fetch per-device results for'), includeOutput: z.boolean().optional().describe('Also fetch StdOut/StdErr for that device (default false)') } },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
