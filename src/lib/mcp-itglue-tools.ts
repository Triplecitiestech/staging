...
import { z } from 'zod'
import { ItGlueClient } from '@/lib/it-glue'
...
export function registerItGlueTools(server: any) {
  ...
  server.registerTool('itglue_org_documents', { title: 'IT Glue: org documents', description: 'List documents (SOPs / runbooks / KB articles) for an organization id. Does NOT return passwords.', inputSchema: { organizationId: z.string().describe('IT Glue organization id'), documentFolderId: z.string().optional().describe('Optional document folder id filter') } },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async ({ organizationId, documentFolderId }: any) => { try { const c = itglue(); ensureConfigured(c); return ok(await c.getDocuments(organizationId, documentFolderId)) } catch (e) { return fail(e) } })
