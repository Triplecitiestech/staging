...
export const TOOL_FACTS: Record<string, ToolFacts> = {
  // ── Autotask: the generic catalogue-driven surface ────────────────────────
  // These five cover EVERY entity in the generated catalogue, which is what
  // ended the per-entity gap. `staged: false` is the honest value: the gate
  // applies PER ENTITY (instance configuration is gated, operational records
  // are not), so a flat true would misreport a direct ticket-charge write as
  // needing approval. The constraint below states the real rule.
  autotask_entity_catalogue: r(
    'Build-time snapshot of the API surface — says what the connector covers, never what the vendor can do right now',
    'Regenerated by npm run gen:autotask-catalogue; the coverage CI check fails the build on anything it lists that no tool exposes',
  ),
  autotask_entity_query: r(
    'One API page, max 500 rows',
    'Prefer a dedicated read tool where one exists — those resolve labels, links and activity gaps this does not',
  ),
  autotask_entity_create: atWrite(
    'Fields validated against LIVE entityInformation before anything is sent; read-back verified per field',
    'Operational records write directly; instance CONFIGURATION and any unclassified entity go through the staged approval gate and return a stagedWriteId instead',
    'Prefer an ergonomic tool where one exists — autotask_create_task knows the four-field assignment group, this does not',
  ),
  autotask_entity_update: atWrite(
    'PATCHes only the named fields — no GET-and-merge',
    'Read-back verified per field: a value Autotask accepts and discards is PRECONDITION_FAILED, never success',
    'Operational records write directly; instance CONFIGURATION and any unclassified entity go through the staged approval gate',
  ),
  autotask_entity_delete: atWrite(
    'Verified by re-read — a delete Autotask accepts but does not perform is reported as a failure',
    'TimeEntries, Contacts and Companies deletes are refused by a dated TCT guardrail, not by Autotask',
  ),
  // ── Autotask: ticket charges ──────────────────────────────────────────────
  autotask_add_ticket_charge: atWrite(
    'chargeType resolved from the LIVE picklist, never hardcoded',
    'unitPrice is what the customer pays; unitCost is what TCT paid. billableAmount and extendedCost are computed by Autotask and returned from the read-back',
  ),
  autotask_update_ticket_charge: atWrite(
    'Read-back verified per field',
    'Warns when the charge had already been billed — Autotask does not reissue the invoice it is on',
  ),
  autotask_delete_ticket_charge: atWrite(
    'Verified by re-read',
    'Refuses an already-billed charge unless force is passed: deleting one removes money from an invoice that may already have been sent',
  ),
  autotask_ticket_charges: r(
    'Ticket TIME entries are separate — autotask_ticket_time_entries covers those',
    'Totals are Autotask\'s own computed billableAmount / extendedCost summed across the returned lines',
  ),
  // ── Meta ─────────────────────────────────────────────────────────────────
