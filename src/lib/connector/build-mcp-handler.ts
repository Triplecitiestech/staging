...
import { runSearchTickets } from '@/lib/connector/ticket-search-tool'
import { TICKET_GROUP_BY } from '@/lib/connector/ticket-search-shape'
  ...
  {
    {
      ...
      server.registerTool('autotask_search_tickets', { title: 'Autotask: search tickets (business-wide)', description: 'Search tickets across ALL companies (or one, via companyId) with server-side filters — status ids (from autotask_ticket_statuses), priority, queueId, assignedResourceId, openOnly, and a date window on createDate / lastActivityDate / completedDate. FOR "HOW MANY" OR "BREAKDOWN" QUESTIONS USE count_only OR group_by — they return a few KB instead of the full rows (an all-company open-ticket pull is ~500 KB). count_only=true returns { count, truncated } only. group_by (company | status | priority | queue | assignedResource) returns { groupBy, total, truncated, groupCount, groups[{ id, name, count }] } sorted by count descending, with names resolved; every filter still applies and the full filtered set is paginated before counting — if the fetch truncates, groups is empty and truncated=true (a partial count is never returned as a total). fields=[...] keeps only those ticket fields per row (id, ticketNumber, ticketUrl always included) and adds companyName — use it to pull full rows only for the tickets you will actually discuss. Note openOnly means "no completedDate", so statuses such as "Complete - No Notify" still count as open; filter by status ids for truly-worked tickets. With none of count_only / group_by / fields, returns reporting fields PLUS every SLA field (serviceLevelAgreementID + name via autotask_list_slas, serviceLevelAgreementHasBeenMet, first-response/resolution-plan/resolved actual + due datetimes) so you can compute "did we meet SLA" and "about to breach" (breach = dueDateTime − now). Auto-paginates by splitting the date window; returns { count, truncated, activityGapAdvisory, tickets[] } and each ticket carries lastActivityDate + activityGap. If truncated=true, narrow the filters or window. Ticket FIELDS only — no notes or time entries — so this read is never evidence that work was not done; use autotask_ticket_activity for that. Read-only.', inputSchema: { companyId: z.number().int().optional().describe('Limit to one Autotask company id (omit for ALL companies)'), status: z.array(z.number().int()).optional().describe('One or more status ids'), priority: z.number().int().optional().describe('Priority id'), queueId: z.number().int().optional().describe('Queue id'), assignedResourceId: z.number().int().optional().describe('One technician (from autotask_find_resource / autotask_list_resources)'), openOnly: z.boolean().optional().describe('Only not-completed tickets (no completedDate)'), dateField: z.enum(['createDate', 'lastActivityDate', 'completedDate']).optional().describe('Which date the from/to window filters (default createDate)'), from: z.string().optional().describe('Window start YYYY-MM-DD (default 90 days ago)'), to: z.string().optional().describe('Window end YYYY-MM-DD (default now)'), max: z.number().int().optional().describe('Max rows (default 2000, hard cap 5000)'), count_only: z.boolean().optional().describe('Return only { count, truncated }. Use for "how many" questions. Takes precedence over group_by and fields.'), group_by: z.enum(TICKET_GROUP_BY).optional().describe('Return per-group counts with resolved names, sorted by count desc, plus total. Use for "breakdown" / "per client" / "per tech" questions.'), fields: z.array(z.string()).optional().describe('Return only these ticket fields per row (id, ticketNumber, ticketUrl always included; companyName added). Unknown names are ignored.') } }, async ({ companyId, status, priority, queueId, assignedResourceId, openOnly, dateField, from, to, max, count_only, group_by, fields }) => { try { return ok(await runSearchTickets(autotask(), { companyId, status, priority, queueId, assignedResourceId, openOnly, dateField, from: from ? new Date(from) : undefined, to: to ? new Date(to) : undefined, max }, { count_only, group_by, fields }, { row: ticketWithGap, advisory: ticketReadAdvisory })) } catch (e) { return failAt(e) } })
