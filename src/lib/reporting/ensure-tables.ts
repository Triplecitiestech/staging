/**
 * Auto-creates reporting tables if they don't exist.
 * Called automatically by every reporting job before it runs.
...
// Cache: once we've verified tables exist in this process, skip re-checking
let tablesVerified = false;

/**
 * Ensure all reporting tables exist. Auto-creates any that are missing.
 * Fast path: if already verified in this process lifetime, returns immediately.
 */
export async function ensureReportingTables(): Promise<void> {
  ...
  console.log(`[reporting] Auto-creating ${missing.length} missing tables: ${missing.join(', ')}`);
  ...
  for (const stmt of MIGRATION_STATEMENTS) {
    try {
      ...
      if (!msg.includes('already exists')) {
        console.error(`[reporting] Migration statement failed: ${msg.slice(0, 200)}`);
      }
    }
  }

  // Record in Prisma migration history
  try {
