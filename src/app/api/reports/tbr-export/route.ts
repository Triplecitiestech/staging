...
import { isAutomatedTicket } from '@/lib/reporting/ticket-classification';
  ...
  try {
    ...
    const data: TbrExportData = {
      ...
      definitions: {
        ...
        humanVsMonitoring: 'Human support vs proactive monitoring uses the platform\'s shared ticket classifier: ticket Source "Monitoring Alert" = automated (RMM, EDR, SaaS Alerts), all interactive sources = human; a source-misfiled ticket that a technician was assigned and worked counts as human. Backlog and resolution stats cover human tickets only.',
...
function aggregateTickets(
  ...
  for (const t of tickets) {
    ...
    // Human-vs-automated via the shared classifier (source-primary with the
    // misfiled-ticket rescue) — the same rule every report surface uses.
    const monitoring = isAutomatedTicket({
      source: t.source ?? null,
      sourceLabel: t.source != null ? maps.sourceMap.get(t.source) ?? null : null,
      queueId: t.queueID ?? null,
      queueLabel: t.queueID != null ? maps.queueMap.get(t.queueID) ?? null : null,
    ...
    if (completed) {
      ...
      // Resolution stats are support metrics — human tickets only. Automated
      // auto-resolves (completed == created to the millisecond) would blend
      // meaningless zeros into the averages.
      const resHours = (completed.getTime() - created.getTime()) / 3_600_000;
      if (resHours >= 0 && !monitoring) {
    ...
    // Open backlog (created in period, not closed) — human tickets only:
    // an unresolved automated alert ticket is a monitoring record, not
    // work waiting on a technician.
    if (!isClosed && !monitoring) {
      openCount++;
