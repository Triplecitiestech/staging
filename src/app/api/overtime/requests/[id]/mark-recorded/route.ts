...
import { markOvertimeRecordedInPayroll } from '@/lib/overtime/service'
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const actualHoursWorked =
      typeof body?.actualHoursWorked === 'number' ? body.actualHoursWorked : null

    try {
      const result = await markOvertimeRecordedInPayroll({
        requestId: id,
