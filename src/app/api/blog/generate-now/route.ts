...
/**
 * Manual blog generation endpoint (GET)
 * Can be triggered from admin UI
 *
 * GET /api/blog/generate-now
 */
export async function GET() {
  try {
    ...
    if (articles.length === 0) {
      ...
      return NextResponse.json({
        success: false,
        error: 'No articles found. Make sure RSS feeds are configured and have recent content.'
      }, { status: 400 });
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Blog post generated successfully! Approval email sent to ${approvalEmail}`,
      blogPost: {
        id: blogPost.id,
