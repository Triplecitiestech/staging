...
const handler = createMcpHandler(
  (mcpServer) => {
    ...
    server.registerTool('autotask_ticket_notes', { title: 'Autotask: ticket notes', description: 'List notes on an Autotask ticket by numeric ID.', inputSchema: { ticketId: z.number().int().describe('Autotask ticket ID') } }, async ({ ticketId }) => { try { return ok(await autotask().getTicketNotes(ticketId)) } catch (e) { return failAt(e) } })
    server.registerTool('autotask_ticket_time_entries', { title: 'Autotask: ticket time entries', description: 'Read all TIME ENTRIES logged on a ticket: hours worked, billable hours (hoursToBill), billable vs non-billable (isNonBillable/billable), who logged each (resource name + email), start/stop, billing code, role, and the entry summary + internal notes. Read-only. No internal cost/rate data is exposed (Autotask time entries carry none).', inputSchema: { ticketId: z.number().int().describe('Autotask ticket ID') } }, async ({ ticketId }) => { try { return ok(await autotask().getTicketTimeEntriesDetailed(ticketId)) } catch (e) { return failAt(e) } })
