...
export async function POST(request: NextRequest) {
  try {
    ...
    if (companyId) {
      ...
      // Find-or-create the local company from Autotask. Shared with the SOC
      // ingest path so there's exactly one "company from Autotask id" routine.
      const { findOrCreateCompanyFromAutotask } = await import('@/lib/reporting/sync');
      company = await findOrCreateCompanyFromAutotask(atCompany);
    }
